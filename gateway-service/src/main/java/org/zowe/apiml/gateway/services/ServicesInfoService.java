...
public class ServicesInfoService {
    ...
    private String getBaseUrl(ApiInfo apiInfo, InstanceInfo instanceInfo) {
        ...
        return String.format("%s://%s%s", gatewayAddress.getScheme(), gatewayAddress.getHostname(), getBasePath(apiInfo, instanceInfo));
    }
    ...
    private ServiceInfo getServiceInfo(String serviceId, List<ServiceInstance> serviceInstances) {
        ...
        return ServiceInfo.builder().serviceId(serviceId).status(getStatus(primaryInstances)).apiml(getApiml(primaryInstances)).instances(getInstances(primaryInstances.stream().filter(EurekaServiceInstance.class::isInstance).map(EurekaServiceInstance.class::cast).map(EurekaServiceInstance::getInstanceInfo).toList())).build();
    }

    private ServiceInfo.Apiml getApiml(List<ServiceInstance> serviceInstances) {
        return ServiceInfo.Apiml.builder().apiInfo(getApiInfos(serviceInstances)).service(getService(serviceInstances)).authentication(getAuthentication(serviceInstances)).build();
    }
    ...
    private List<ServiceInfo.ApiInfoExtended> getApiInfos(List<ServiceInstance> serviceInstances) {
        ...
        for (InstanceInfo instanceInfo : appInstances) {
            ...
            completeList.addAll(apiInfoList.stream().map(apiInfo -> ServiceInfo.ApiInfoExtended.builder().apiId(apiInfo.getApiId()).basePath(getBasePath(apiInfo, instanceInfo)).baseUrl(getBaseUrl(apiInfo, instanceInfo)).gatewayUrl(apiInfo.getGatewayUrl()).swaggerUrl(getGatewayUrl(apiInfo.getSwaggerUrl(), instanceInfo.getAppName().toLowerCase(), ServiceType.API, eurekaMetadataParser.parseRoutes(instanceInfo.getMetadata()))).documentationUrl(apiInfo.getDocumentationUrl()).version(apiInfo.getVersion()).codeSnippet(apiInfo.getCodeSnippet()).isDefaultApi(apiInfo.isDefaultApi()).build()).toList());
        }
        return completeList.stream().collect(groupingBy(apiInfo -> new AbstractMap.SimpleEntry<>(apiInfo.getApiId(), getMajorVersion(apiInfo)), minBy(Comparator.comparingInt(ServiceInfoUtils::getMajorVersion)))).values().stream().map(Optional::get).toList();
    }
    ...
    private ServiceInfo.Service getService(List<ServiceInstance> serviceInstances) {
        ...
        return ServiceInfo.Service.builder().title(instanceInfo.getMetadata().get(SERVICE_TITLE)).description(instanceInfo.getMetadata().get(SERVICE_DESCRIPTION)).homePageUrl(getGatewayUrl(instanceInfo.getHomePageUrl(), instanceInfo.getAppName().toLowerCase(), ServiceType.UI, routes)).build();
    }
    ...
    private String getGatewayUrl(String url, String serviceId, ServiceType type, RoutedServices routes) {
        if (url == null)
            return null;
        try {
            return transformService.transformURL(type, serviceId, url, routes, false);
    ...
    private InstanceInfo.InstanceStatus getStatus(List<ServiceInstance> instances) {
        if (instances.stream().filter(EurekaServiceInstance.class::isInstance).map(EurekaServiceInstance.class::cast).anyMatch(instance -> instance.getInstanceInfo().getStatus().equals(InstanceInfo.InstanceStatus.UP))) {
