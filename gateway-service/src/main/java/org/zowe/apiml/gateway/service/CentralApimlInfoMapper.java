...
public class CentralApimlInfoMapper {
    ...
    private CentralServiceInfo mapServices(ServiceInfo gws) {
        return CentralServiceInfo.builder().serviceId(gws.getServiceId()).status(gws.getStatus()).apiId(extractApiId(gws.getApiml())).customMetadata(extractMetadata(gws)).build();
    }

    private Map<String, String> extractMetadata(ServiceInfo gws) {
        return Optional.ofNullable(gws.getInstances()).orElseGet(Collections::emptyMap).entrySet().stream().findFirst().map(Map.Entry::getValue).filter(i -> !isEmpty(i.getCustomMetadata())).map(ServiceInfo.Instances::getCustomMetadata).orElse(Collections.emptyMap()).entrySet().stream().filter(entry -> metadataKeysAllowList.contains(entry.getKey())).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
