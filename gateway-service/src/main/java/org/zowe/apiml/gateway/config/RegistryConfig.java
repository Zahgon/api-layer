...
public class RegistryConfig {
    ...
    @Bean
    ServiceAddress gatewayServiceAddress(@Value("${apiml.service.externalUrl:#{null}}") String externalUrl, @Value("${server.attlsServer.enabled:false}") boolean serverAttlsEnabled, @Value("${server.attlsClient.enabled:false}") boolean clientAttlsEnabled, @Value("${server.ssl.enabled:true}") boolean sslEnabled, @Value("${apiml.service.hostname:localhost}") String hostname, @Value("${server.port}") int port) throws URISyntaxException {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
