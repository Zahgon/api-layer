...
public class ValidateAPIController {
     ...
     */
    @GetMapping(value = "/conformance/{serviceId}", produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Checks service conformance.", operationId = "checkConformanceUsingGET", description = "Accepts serviceID and checks conformance criteria.", security = { @SecurityRequirement(name = "CookieAuth"), @SecurityRequirement(name = "Bearer") })
    @ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Service conforms to the criteria", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(type = "object", maxProperties = 1), schemaProperties = { @SchemaProperty(name = "message", schema = @Schema(type = "string", example = "Service {serviceId} fulfills all checked conformance criteria")) })), @ApiResponse(responseCode = "400", description = "Service does not conform to the criteria", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(type = "object", maxProperties = 2), schemaProperties = { @SchemaProperty(name = "error", schema = @Schema(type = "string")), @SchemaProperty(name = "details", array = @ArraySchema(schema = @Schema(name = "items", type = "string"))) })) })
    public ResponseEntity<String> checkConformance(@Parameter(in = ParameterIn.PATH, required = true, description = "Service ID of the service to check") @PathVariable String serviceId, Authentication authentication) {
    ...
    private void validateSwaggerDocument(String serviceId, ConformanceProblemsContainer foundNonConformanceIssues, Map<String, String> metadata, Optional<String> swaggerUrl, String token) throws ValidationException {
        ...
        if (parserResponses != null)
            foundNonConformanceIssues.add(CONFORMANCE_PROBLEMS, parserResponses);
    ...
    @Operation(summary = "Legacy endpoint for checking service conformance.", operationId = "checkValidateLegacyUsingPOST", description = "Mapping so the old endpoint keeps working.", security = { @SecurityRequirement(name = "CookieAuth"), @SecurityRequirement(name = "Bearer") }, requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(mediaType = MediaType.TEXT_PLAIN_VALUE, schema = @Schema(example = "serviceID=serviceId"))))
    @ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Service conforms to the criteria", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(type = "object", maxProperties = 1), schemaProperties = { @SchemaProperty(name = "message", schema = @Schema(type = "string", example = "Service {serviceId} fulfills all checked conformance criteria")) })), @ApiResponse(responseCode = "400", description = "Service does not conform to the criteria", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(type = "object", maxProperties = 2), schemaProperties = { @SchemaProperty(name = "error", schema = @Schema(type = "string")), @SchemaProperty(name = "details", array = @ArraySchema(schema = @Schema(name = "items", type = "string"))) })) })
