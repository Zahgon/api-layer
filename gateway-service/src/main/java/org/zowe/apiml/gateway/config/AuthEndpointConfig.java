...
public class AuthEndpointConfig {
    ...
    private String[] HEADERS_TO_RESEND = { SET_COOKIE, CONTENT_TYPE, AUTH_FAIL_HEADER };

    private final WebClient webClient;

    private final WebClient webClientClientCert;

    public AuthEndpointConfig(WebClient webClient, @Qualifier("webClientClientCert") WebClient webClientClientCert, ReactiveLoadBalancer.Factory<ServiceInstance> serviceInstanceFactory) {
    ...
    private WebClient createLoadBalanced(WebClient webClient, ReactiveLoadBalancer.Factory<ServiceInstance> serviceInstanceFactory) {
        return webClient.mutate().filter(new ReactorLoadBalancerExchangeFilterFunction(serviceInstanceFactory, Collections.emptyList())).build();
    }
    ...
    private WebClient.RequestBodySpec getWebclient(ServerRequest serverRequest, String path) {
        ...
        var request = client.method(serverRequest.method()).uri("lb://zaas/zaas" + path).headers(headers -> headers.addAll(serverRequest.headers().asHttpHeaders())).headers(headers -> headers.remove(CLIENT_CERT_HEADER));
    ...
    private Mono<ServerResponse> resend(ServerRequest request, String path, String body) {
        ...
        return getWebclient(request, path).body(bodyInserter).exchangeToMono(clientResponse -> {
            var response = ServerResponse.status(clientResponse.statusCode());
            response.headers(httpHeaders -> Arrays.stream(HEADERS_TO_RESEND).forEach(headerName -> httpHeaders.addAll(headerName, clientResponse.headers().header(headerName))));
    ...
    private HandlerFunction<ServerResponse> resendTo(String path) {
        return request -> request.bodyToMono(String.class).switchIfEmpty(Mono.just("")).flatMap(body -> resend(request, path, body)).doOnError(e -> log.debug("Cannot resend authentication call to the ZAAS", e));
    }
