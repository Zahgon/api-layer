...
public class ZaasSchemeTransformRest implements ZaasSchemeTransform {
    ...
    private <R> Mono<AbstractAuthSchemeFactory.AuthorizationResponse<R>> requestWithHa(Class<R> responseClass, Iterator<ServiceInstance> serviceInstanceIterator, Function<ServiceInstance, WebClient.RequestHeadersSpec<?>> requestCreator, // to be accessible and updatable in all lambdas below
    AtomicReference<Optional<Exception>> mostCriticalException) {
        ...
        return requestCreator.apply(zaasInstance).exchangeToMono(clientResp -> switch(clientResp.statusCode().value()) {
            case SC_UNAUTHORIZED ->
                Mono.just(new AbstractAuthSchemeFactory.AuthorizationResponse<>(clientResp.headers(), null));
            case SC_OK ->
                clientResp.bodyToMono(responseClass).map(zaasResponseBody -> new AbstractAuthSchemeFactory.AuthorizationResponse<>(clientResp.headers(), zaasResponseBody));
            case SC_INTERNAL_SERVER_ERROR ->
                callNext.apply(new ZaasInternalErrorException(zaasInstance, "An internal exception occurred in ZAAS service. Check its configuration of instance " + zaasInstance.getInstanceId() + "."));
            default ->
                callNext.apply(new ServiceNotAccessibleException(SERVICE_IS_UNAVAILABLE_MESSAGE));
        }).doOnError(t -> log.debug("Error on calling ZAAS service instance {}: {}", zaasInstance.getInstanceId(), t.getMessage())).onErrorResume(e -> callNext.apply(new ServiceNotAccessibleException(SERVICE_IS_UNAVAILABLE_MESSAGE)));
    }
    ...
    private <R> Mono<AbstractAuthSchemeFactory.AuthorizationResponse<R>> call(Class<R> responseClass, Function<ServiceInstance, WebClient.RequestHeadersSpec<?>> requestBuilder) {
        return discoveryClient.getInstances(CoreService.ZAAS.getServiceId()).collectList().flatMap(zaasInstances -> {
            ...
            return requestWithHa(responseClass, i, requestBuilder, new AtomicReference<>(Optional.empty())).switchIfEmpty(Mono.just(new AbstractAuthSchemeFactory.AuthorizationResponse<>(null, null)));
        });
    }
    ...
    private WebClient.RequestHeadersSpec<?> createRequest(RequestCredentials requestCredentials, String uri, String jsonBody) {
        ...
        Optional.ofNullable(requestCredentials.getHeaders()).orElse(Collections.emptyMap()).forEach(requestBuild::header);
        Optional.ofNullable(requestCredentials.getCookies()).orElse(Collections.emptyMap()).forEach(requestBuild::cookie);
