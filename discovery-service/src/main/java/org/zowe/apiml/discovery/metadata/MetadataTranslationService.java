...
public class MetadataTranslationService {
    ...
    private void translateRoutes(Map<String, String> metadata) {
        Map<String, String> newRoutes = metadata.entrySet().stream().filter(entry -> entry.getKey().contains(ROUTES_V1)).collect(Collectors.toMap(this::translateRouteMapKey, Map.Entry::getValue));
    ...
    private String translateRouteMapKey(Map.Entry<String, String> map) {
        return map.getKey().replace(ROUTES_V1, ROUTES).replace(ROUTES_GATEWAY_URL_V1, ROUTES_GATEWAY_URL).replace(ROUTES_SERVICE_URL_V1, ROUTES_SERVICE_URL);
    }
