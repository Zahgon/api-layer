...
public class ServiceDefinitionProcessor {
    ...
    private CatalogUiTile getTile(StaticRegistrationResult context, String ymlFileName, Map<String, CatalogUiTile> tiles, Service service) {
        if (service.getCatalogUiTileId() != null) {
            ...
            if (tile == null) {
                final Message msg = apimlLog.log(ERROR_PARSING_STATIC_DEFINITION_DATA, ymlFileName, String.format("The API Catalog UI tile ID %s is invalid. The service %s will not have API Catalog UI tile", service.getCatalogUiTileId(), service.getServiceId()));
    ...
    private InstanceInfo buildInstanceInfo(StaticRegistrationResult context, Service service, CatalogUiTile tile, String instanceBaseUrl) throws ServiceDefinitionException {
        ...
        try {
            ...
            if (url.getHost().isEmpty()) {
                ...
                log.debug("Adding static instance {} for service ID {} mapped to URL {}", instanceId, serviceId, url);
            ...
            throw new ServiceDefinitionException(String.format("Metadata creation failed. The instance of %s will not be created: %s", serviceId, mve));
        }
    }
