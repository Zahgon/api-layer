...
public class HttpsFactory {
    ...
    private SSLContext createIgnoringSslContext() {
        try {
            ...
            return new SSLContextBuilder().loadTrustMaterial(null, (certificate, authType) -> true).loadKeyMaterial(emptyKeystore, null).setProtocol(config.getProtocol()).build();
        } catch (Exception e) {
            apimlLog.log("org.zowe.apiml.common.errorInitSsl", e.getMessage());
            throw new HttpsConfigError("Error initializing SSL/TLS context: " + e.getMessage(), e, ErrorCode.SSL_CONTEXT_INITIALIZATION_FAILED, config);
        }
    }
        ...
        if (StringUtils.isNotEmpty(config.getTrustStore())) {
            ...
            if (!SecurityUtils.isKeyring(config.getTrustStore())) {
                if (config.getTrustStorePassword() == null) {
                    ...
                    throw new HttpsConfigError("server.ssl.trustStorePassword configuration parameter is not defined", ErrorCode.TRUSTSTORE_PASSWORD_NOT_DEFINED, config);
                }
            ...
            if (config.isTrustStoreRequired()) {
                ...
                throw new HttpsConfigError("server.ssl.trustStore configuration parameter is not defined but trust store is required", ErrorCode.TRUSTSTORE_NOT_DEFINED, config);
            } else {
    ...
    private void loadKeystoreMaterial(SSLContextBuilder sslContextBuilder) throws UnrecoverableKeyException, NoSuchAlgorithmException, KeyStoreException, CertificateException, IOException {
        if (StringUtils.isEmpty(config.getKeyStore())) {
            ...
            throw new HttpsConfigError("server.ssl.keyStore configuration parameter is not defined", ErrorCode.KEYSTORE_NOT_DEFINED, config);
        }
        if (config.getKeyStorePassword() == null) {
            apimlLog.log("org.zowe.apiml.common.keystorePasswordNotDefined");
            throw new HttpsConfigError("server.ssl.keyStorePassword configuration parameter is not defined", ErrorCode.KEYSTORE_PASSWORD_NOT_DEFINED, config);
        }
        log.info("Loading keystore file: " + config.getKeyStore());
        File keyStoreFile = new File(config.getKeyStore());
        sslContextBuilder.loadKeyMaterial(keyStoreFile, config.getKeyStorePassword(), config.getKeyPassword(), getPrivateKeyStrategy());
    }
    ...
    private void loadKeyringMaterial(SSLContextBuilder sslContextBuilder) throws UnrecoverableKeyException, NoSuchAlgorithmException, KeyStoreException, CertificateException, IOException {
        ...
        sslContextBuilder.loadKeyMaterial(keyRingUrl, config.getKeyStorePassword(), config.getKeyPassword(), getPrivateKeyStrategy());
    }
    ...
    private synchronized SSLContext createSecureSslContext() {
        ...
        try {
            ...
            throw new HttpsConfigError("Error initializing SSL Context: " + e.getMessage(), e, ErrorCode.HTTP_CLIENT_INITIALIZATION_FAILED, config);
        }
    }
    ...
    private ConnectionSocketFactory getSSLConnectionSocketFactory() {
        return new SSLConnectionSocketFactory(createSecureSslContext(), config.getEnabledProtocols(), null, getHostnameVerifier());
    }
