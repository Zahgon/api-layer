...
public class ReactiveAuthenticationController {
    ...
    @Operation(summary = "Authenticate mainframe user credentials and return authentication token.", tags = { "Security" }, operationId = "loginUsingPOST", description = """
        ...
        """, requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(schemaProperties = { @SchemaProperty(name = "username", schema = @Schema(type = "string")), @SchemaProperty(name = "password", schema = @Schema(type = "string")), @SchemaProperty(name = "newPassword", schema = @Schema(type = "string")) }), description = "Specifies the user credentials to be authenticated. If newPassword is provided and the password is valid, the password is changed to newPassword"))
    @ApiResponses(value = { @ApiResponse(responseCode = "204", description = //
    "Cookie named apimlAuthenticationToken contains authentication\n" + "token."), @ApiResponse(responseCode = "401", description = "Invalid credentials.") })
    public Mono<ResponseEntity<Object>> login(ServerWebExchange exchange, ServerHttpRequest request) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }

    private Mono<ResponseEntity<Object>> authWithBody(ServerWebExchange exchange, ServerHttpRequest request) {
        return readLoginRequestFromBody(request).flatMap(loginRequest -> {
            ...
            return authAdapter.authenticate(new UsernamePasswordAuthenticationToken(loginRequest.getUsername(), loginRequest)).map(authentication -> replyWithJwt(exchange, authentication));
        });
    }
    ...
    @GetMapping("/query")
    @Operation(summary = "Validate the authentication token.", tags = { "Security" }, operationId = "validateUsingGET", description = """
        ...
        """)
    @ApiResponses(value = { @ApiResponse(responseCode = "200", description = "OK", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Authentication.class))), @ApiResponse(responseCode = "401", description = "Invalid credentials.") })
    public Mono<ResponseEntity<QueryResponse>> query() {
        throw new UnsupportedOperationException("STUB: not implemented");
    }

    @Operation(summary = "Invalidate mainframe user session.", tags = { "Security" }, operationId = "logoutUsingPOST", description = """
            Use the `/logout` API to invalidate mainframe user session.
        
            The cookie named `apimlAuthenticationToken` will be removed.
        """)
    @ApiResponses(value = { @ApiResponse(responseCode = "204", description = "Invalidated user session") })
    @PostMapping("/logout")
    ...
    @Operation(summary = "Refresh authentication token.", tags = { "Security" }, operationId = "RefreshTokenUsingPOST", description = """
            ...
            *Header example:* Authorization: Bearer *token*
        """)
    @ApiResponses(value = { @ApiResponse(responseCode = "204", description = "Authenticated - Refreshed Personal Access Token"), @ApiResponse(responseCode = "401", description = "Zowe token is not provided, is invalid or is expired."), @ApiResponse(responseCode = "403", description = "A client certificate is not provided or is expired."), @ApiResponse(responseCode = "404", description = "Not Found. The endpoint is not enabled or not properly configured"), @ApiResponse(responseCode = "500", description = "Process of refreshing token has failed unexpectedly.") })
