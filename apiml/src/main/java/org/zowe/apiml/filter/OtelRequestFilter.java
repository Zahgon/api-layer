...
public class OtelRequestFilter implements WebFilter, GlobalFilter, Ordered {
    ...
    private Mono<Void> filterInternal(ServerWebExchange exchange, Function<ServerWebExchange, Mono<Void>> filter) {
        ...
        return filter.apply(exchange).// in all cases (success / error) issue the log message
        doFinally(signalType -> otelContext.issue()).// update response codes
        then(Mono.fromRunnable(() -> Optional.ofNullable(exchange.getResponse()).map(ServerHttpResponse::getStatusCode).map(HttpStatusCode::value).ifPresent(otelContext::responseCode)));
    }
