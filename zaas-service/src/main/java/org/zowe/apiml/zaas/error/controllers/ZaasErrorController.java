...
public class ZaasErrorController implements ErrorController {
    ...
    private Message getMessageByStatus(HttpServletRequest request, int status) {
        switch(status) {
            ...
            case SC_INTERNAL_SERVER_ERROR:
                ...
                return messageService.createMessage("org.zowe.apiml.common.internalRequestError", ErrorUtils.getForwardUri(request), ExceptionUtils.getMessage(exc), ExceptionUtils.getRootCauseMessage(exc));
            default:
    ...
    private int getStatus(HttpServletRequest request) {
        return Optional.ofNullable((Integer) request.getAttribute(RequestDispatcher.ERROR_STATUS_CODE)).orElse(SC_INTERNAL_SERVER_ERROR);
    }
